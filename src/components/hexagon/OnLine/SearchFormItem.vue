<template>
  ...
  <a-form-item v-else :labelCol="labelCol" :class="'hexagon-online-search'">
    <template #label>
      <span :title="item.label" class="label-text">{{ item.label }}</span>
    </template>
...
<style lang="less" scoped>
  ...
  /* 查询条件左对齐样式设置 */
  .hexagon-online-search :deep(.ant-form-item-label) {
    flex: 0 0 auto !important;
    width: auto;
  }
  .hexagon-online-search :deep(.ant-form-item-control) {
    max-width: 100%;
  }

  /* label显示宽度 超出显示... */
  .hexagon-online-search :deep(.label-text) {
