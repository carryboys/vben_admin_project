...
<script lang="ts" setup>
  ...
  import { queryCodePropertyTreeList, searchByKeywords } from '../code.property.api';
  ...
  // 加载属性信息
  function loadCodePropertyTreeData() {
    loading.value = true;
    treeData.value = [];
    queryCodePropertyTreeList()
      .then((res) => {
        if (res.success) {
      ...
      searchByKeywords({ keyWord: value, myDeptSearch: '1' })
        .then((result) => {
          if (Array.isArray(result)) {
            ...
            createMessage.warning('未查询到属性信息');
            treeData.value = [];
          }
        })
        .finally(() => (loading.value = false));
