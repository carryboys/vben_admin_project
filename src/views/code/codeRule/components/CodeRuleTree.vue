...
<script lang="ts" setup>
  ...
  import { queryCodeRuleTreeList, searchByKeywords } from '../code.rule.api';
  ...
  // 加载规则信息
  function loadCodeRuleTreeData() {
    loading.value = true;
    treeData.value = [];
    queryCodeRuleTreeList()
      .then((res) => {
        if (res.success) {
      ...
      searchByKeywords({ keyWord: value, myDeptSearch: '1' })
        .then((result) => {
          if (Array.isArray(result)) {
            ...
            createMessage.warning('未查询到规则信息');
            treeData.value = [];
          }
        })
        .finally(() => (loading.value = false));
