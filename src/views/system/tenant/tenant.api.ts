...
enum Api {
  ...
  list = '/sys/tenant/list.json',
  save = '/sys/tenant/add.json',
  edit = '/sys/tenant/edit.json',
  get = '/sys/tenant/queryById.json',
  delete = '/sys/tenant/delete.json',
  deleteBatch = '/sys/tenant/deleteBatch.json',
  getCurrentUserTenants = '/sys/tenant/getCurrentUserTenant.json',
}
    ...
    onOk: () => {
      return (
        defHttp
          ...
          .get({ url: Api.deleteBatch, data: params }, { joinParamsToUrl: true })
          .then(() => {
            handleSuccess();
          })
      );
